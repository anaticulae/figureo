...
def text_figures(
    ...
    result = [item for item in result if utilo.rect_width(item.bounding) > 25.0]
...
def figures_missing(items, done) -> list:
    ...
    figures = utilo.select_type(items, selector=pdfminer.layout.LTFigure)
    result = []
    for item in figures:
        if utilo.rectangles_intersecting(done, item.bbox):
            # LTFigure is already part of a text figure
            continue
        matching = [
            test.bbox
            for test in items
            if utilo.rect_intersecting(item.bbox, test.bbox)
        ]
        bounding = utilo.rect_max([item.bbox] + matching)
        figure = iamraw.Figure(bounding=bounding)
        result.append(figure)
    return result
...
def bounding_valid(bounding: tuple, items, width_min, height_min, area_min) -> bool:  # yapf:disable
    ...
    if not textonly(bounding, items):
        return True
    if utilo.rect_size(bounding) < area_min:
        return False
    if utilo.rect_width(bounding) >= width_min:
        return True
    if utilo.rect_height(bounding) >= height_min:
        return True
    return False


CONTENT_INVALID_RATE_MAX = configos.HolyTable(items=[
    (5, 0.1),
    (10, 0.2),
    (15, 0.2),
    (20, 0.2),
...
def content_valid(bounding, content, invalids) -> bool:
    ...
    content = [
        ...
        if utilo.rect_intersecting(bounding, item.bbox)
    ]
    invalids = [
...
def splitby_breaker(items, breaker):
    ...
    grouped = utilo.Buckets(border=breaker)
...
CLUSTER_SIZE_MIN = configos.HV_INT_PLUS(default=25)
...
def determine_clusters(
    ...
    bucket = utilo.Buckets(utilo.ranges(0, 1000, 15), sorting=True)
    for item in items:
        ...
        # for _ in utilo.ranges(item.bbox[0], item.bbox[2], step=50):
        for coordinate in utilo.ranges(start, end, step=5):
            bucket.add(utilo.roundme(coordinate))
    # select areas with enough items
    content = utilo.groupby_neighbors(bucket)
...
AREA_START_HEIGHT_MAX = configos.HV_FLOAT_PLUS(default=25.0)
# Covering less than this is detected as text line start
AREA_COVERING_RATE_MIN = configos.HV_PERCENT_PLUS(default=35.0)
...
def determine_cluster_rectangle(
    ...
    firstline = [
        item for item in items if utilo.rect_inside(start_area, item.bbox)
    ]
    ...
    result = utilo.rect_max([item.bbox for item in incluster])
    return result


def hanging_textline(area, firstline) -> bool:
    buckets = utilo.Buckets(border=utilo.ranges(
        start=area[0],
        stop=area[2],
        step=20.0,
    ))
    for item in firstline:
        for x in utilo.ranges(start=item.bbox[0], stop=item.bbox[2], step=5.0):
            buckets.add(x)
    # 1/3 2/3
