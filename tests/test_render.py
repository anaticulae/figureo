...
def test_figures_run_bachelor56page27(td, mp):
    ...
    into figure."""
    run_standard(hoverpower.BACHELOR056_PDF, pages=27, mp=mp)
    images = serializeraw.load_image_infos_frompath(
        td.tmpdir.join('rawmaker__images_images'))
    images = utilo.flatten_content(images)
    assert len(images) == 1
    image = images[0]
...
def test_reg_figure_text_in_figure(mp):
    ...
    images = run_standard(hoverpower.MASTER078_PDF, pages=0, mp=mp)
    image = images[0]
...
def test_master31page4(mp):
    images = run_standard(hoverpower.MASTER031_PDF, pages=4, mp=mp)
...
def test_master31page10(mp):
    ...
    assert utilo.nears(selected, expected, diff=4.0)
...
def test_bachelor37page23(mp):
    ...
    assert utilo.nears(boundings[0], expected[0])
    assert utilo.nears(boundings[1], expected[1])
...
@pytest.mark.parametrize('page, expected', [
    ...
    pytest.param(21, [(105.66, 167.68, 517.97, 337.68), (105.66, 386.18, 517.97, 646.18)], id='page21'),
...
def test_bachelor67pagex(page, expected, td, mp):  # pylint:disable=W0613
    ...
    """
    verify(hoverpower.BACHELOR067_PDF, page, expected, mp)
...
def verify(source, page, expected, mp):
    ...
    if isinstance(expected, tuple):
        expected = [expected]
    boxs = utilo.sort_leftright_topdown([figure.bounding for figure in images])
